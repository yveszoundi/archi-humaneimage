...
var humaneImage = {
    "renderViewAsBase64": function (currentView, options) {
        ...
        function fetchPreferences() {
            ...
            try {
                // Older Archi versions
                ret = Java.type('com.archimatetool.editor.ArchiPlugin').PREFERENCES;
                ret.getString('defaultViewFont');
            } catch (e) {
                ret = Java.type("com.archimatetool.editor.ArchiPlugin").getInstance().getPreferenceStore();
            }

            return ret;
        }

        const archiPrefs = fetchPreferences();
        ...
        let cfg = {
            ...
            brightnessContrastAmount      : 80,
            layoutGapTolerance            : 5,
            svgWidth                      : 5,
            svgHeight                     : 5,
            fontSizeCorrectionRatio       : 1,
        ...
        function _hexToRgb(hex) {
            ...
            // Parse the entire hex string into a single integer (base 16)
            const num = parseInt(hexValue, 16);

            // Use bitwise operations to extract the R, G, and B components
            const r = (num >> 16) & 0xFF;
            const g = (num >> 8) & 0xFF;
            const b = num & 0xFF;
        ...
        function _perceivedBrightness(rgb) {
            return (0.299 * rgb.r + 0.587 * rgb.g + 0.114 * rgb.b) / 255;
        }

        function _changeColor(color, amount) { // #FFF not supportet rather use #FFFFFF
        ...
        function _elementIsDiagramModel(e) {
            return e.type == "archimate-diagram-model"
        }
        ...
        function _renderViewAsSvgBase64(currentView, cfg) {
            ...
            function _evaluateBoundaries(activeView, cfg) {
                ...
                $(activeView).children().each(function (e) {
                    if (! (_elementIsConnection(e) || _elementIsDiagramModel(e)) )
                ...
                $(activeView).find().each(function(e) {
                    ...
                    } else if (! _elementIsDiagramModel(e)) {
            ...
            function _drawMultilineText (text, x, y, widthLimit, heightLimit, fontName, fontSize, fontRatio, fontWeight, fontStyle, textAlignment, textPosition, doNotDrawTextIfBoxIsTooSmall, layoutGapTolerance) {
                let fontStyleTextLines = _mapToJavaFontStyle(fontWeight, fontStyle)
                let newText = strings.textLines(text, widthLimit, fontName, fontStyleTextLines, fontSize * fontRatio)
                let textData = ''
                ...
                if (textPosition != TEXT_POSITION.TOP) {     // TOP
                    if (textPosition == TEXT_POSITION.CENTER) { // CENTER
                ...
                if (textAlignment != TEXT_ALIGNMENT.LEFT ) {
                    if (textAlignment == TEXT_ALIGNMENT.CENTER)
                        textAnchor = "middle"
                    else
                        textAnchor = "end"
                }
                ...
                for (let newTextLine of newText) {
                    let horizontalTextPosition = x + 4 //x + 5

                    if (textAlignment != TEXT_ALIGNMENT.LEFT ) {
                        let newTextLineWidth = strings.pixelWidth(newTextLine, fontName, fontStyleTextLines, fontSize)* fontRatio

                        if (textAlignment == TEXT_ALIGNMENT.CENTER) {
                            horizontalTextPosition = (x + (widthLimit /2)) - (newTextLineWidth / 2) //+ 2
                        } else {
                ...
                //    textData = textData + `<tspan y="${dy}" font-weight="${fontWeight}" font-size="${fontSize}" font-style="${fontStyle}">${newTextLine}</tspan>`
                    dy+= (lineHeight )
                }
            ...
            function _drawElement (bounds, element, level, cfg) {
                ...
                let lineColor = _defaultLineColor

                if (element.deriveLineColor) {
                    let lineRgb = _hexToRgb(fillColor)
                    let brightness = _perceivedBrightness(lineRgb)
                    let contrastAmount = cfg.brightnessContrastAmount

                    if (brightness > 0.5)
                        contrastAmount = -1 * cfg.brightnessContrastAmount

                    lineColor = _changeColor(fillColor, contrastAmount)
                } else {
                    lineColor = element.lineColor
                }

                let fontName = element.fontName || _defaultFontName
                let fontSize = (element.fontSize || _defaultFontSize) * cfg.fontSizeCorrectionRatio
                let groupingStyle = ''

                if (element.lineStyle == LINE_STYLE.DASHED) { // dashed
                    groupingStyle = ` stroke-dasharray="${cfg.dashSizeLine}" `
                } else if (element.lineStyle == LINE_STYLE.DOTTED) { // dotted
                    groupingStyle = ` stroke-dasharray="1" `
                }
                ...
                if (element.lineStyle == LINE_STYLE.NONE) {
                    // Don't draw element border
                } else {
                    let fillOpacity = (element.opacity || 255.0).toFixed(1) / 255.0
                    
                    result = `<rect ` + additionalRectAttributes +  ` x = "${adjustedBounds.x}" y="${adjustedBounds.y }" width="${adjustedBounds.width}"  ${groupingStyle}  height="${adjustedBounds.height}" fill="${fillColor}" fill-opacity="${fillOpacity}" stroke-width="${strokeWidth}" stroke="${lineColor}"/>`

                    if (element.prop("SHAPE_TYPE") == "circle") {
                        let cx = adjustedBounds.x + adjustedBounds.width / 2
                        let cy = adjustedBounds.y + adjustedBounds.height / 2
                        let r = adjustedBounds.width / 2
                        result = `<circle r="${r}" cx="${cx}" cy="${cy}" style="fill:${fillColor};stroke:${lineColor};stroke-width:${strokeWidth}" />`
                    }

                    if (element.type == "junction") {
                        let junctionFillColor = lineColor

                        if (element.concept.junctionType == "or")
                            junctionFillColor = "#FFFFFF"

                        // circle representation for branches/decisions
                        // let cx = adjustedBounds.x + adjustedBounds.width / 2
                        // let cy = adjustedBounds.y + adjustedBounds.height / 2
                        // let r = adjustedBounds.width / 2
                        // result = `<circle r="${r}" cx="${cx}" cy="${cy}" style="fill:${junctionFillColor};stroke:${lineColor};stroke-width:${strokeWidth + 1}" />`

                        // polygon representation for branches/decisions
                        let pointList = `${adjustedBounds.x},${adjustedBounds.y + adjustedBounds.height/2}`
                        pointList +=    ` ${adjustedBounds.x + adjustedBounds.width/2},${adjustedBounds.y}`
                        pointList +=    ` ${adjustedBounds.x + adjustedBounds.width},${adjustedBounds.y + adjustedBounds.height/2}`
                        pointList +=    ` ${adjustedBounds.x + adjustedBounds.width/2},${adjustedBounds.y + adjustedBounds.height}`

                        result = `<polygon points="${pointList}" style="fill:${junctionFillColor};stroke:${lineColor};stroke-width:${strokeWidth + 1}" />`
                    }
                }

                let showElementImage = (element.image != undefined || (element.imageSource == 1) || (element.specialization))

                if (showElementImage)  {
                    let img = null

                    if (element.specialization) {
                        if (element.imageSource == IMAGE_SOURCE.SPECIALIZATION)
                            img = model.findSpecialization(element.specialization, element.type).getImage()
                        else
                            img = element.image
                    } else if (element.image) {
                        if (element.imageSource == IMAGE_SOURCE.CUSTOM || element.imageSource == -1)
                            img = element.image
                    }
                    ...
                    if (img) {
                        ...
                        if (imagePosition) {
                            ...
                            } else if (imagePosition == IMAGE_POSITION.TOP_CENTRE) {
                                imageX = adjustedBounds.x + (adjustedBounds.width/2) - (imageWidth/2) 
                                imageY = adjustedBounds.y + 2
                            } else if (imagePosition == IMAGE_POSITION.TOP_RIGHT) {
                                imageX = adjustedBounds.x + adjustedBounds.width - imageWidth - 1
                                imageY = adjustedBounds.y + 2
                            } else if (imagePosition == IMAGE_POSITION.MIDDLE_LEFT) {
                                imageX = adjustedBounds.x + 2
                                imageY = adjustedBounds.y + (adjustedBounds.height / 2) - (imageHeight/2)
                            } else if (imagePosition == IMAGE_POSITION.MIDDLE_CENTRE) {
                                imageX = adjustedBounds.x + (adjustedBounds.width/2) - (imageWidth/2) - 1
                                imageY = adjustedBounds.y + (adjustedBounds.height / 2) - (imageHeight/2)
                            } else if (imagePosition == IMAGE_POSITION.MIDDLE_RIGHT) {
                                imageX = adjustedBounds.x + adjustedBounds.width - imageWidth - 1
                                imageY = adjustedBounds.y + (adjustedBounds.height / 2) - (imageHeight/2)
                            } else if (imagePosition == IMAGE_POSITION.BOTTOM_LEFT) {
                                imageX = adjustedBounds.x + 2
                                imageY = adjustedBounds.y + (adjustedBounds.height) - 2 - imageHeight
                            } else if (imagePosition == IMAGE_POSITION.BOTTOM_CENTRE) {
                                imageX = adjustedBounds.x + (adjustedBounds.width/2) - (imageWidth/2) - 1
                                imageY = adjustedBounds.y + (adjustedBounds.height) - 2 - imageHeight
                            } else if (imagePosition == IMAGE_POSITION.BOTTOM_RIGHT) {
                                imageX = adjustedBounds.x + adjustedBounds.width - imageWidth - 1
                                imageY = adjustedBounds.y + (adjustedBounds.height) - 2 - imageHeight
                            } else if (imagePosition == IMAGE_POSITION.FILL) {
                                imageX = adjustedBounds.x + 8
                                imageY = adjustedBounds.y - 2
                                let wratio  = (1.0 * adjustedBounds.width)  / (1.0 * imageWidth)
                                let hratio  = (1.0 * adjustedBounds.height) / (1.0 * imageHeight)
                                imageWidth  = Math.floor(imageWidth.toFixed(1)  * wratio)
                                imageHeight = Math.floor(imageHeight.toFixed(1) * hratio)
                            }
                        }
                ...
                if (itemText.length != 0) {
                    if (!(element.type == 'junction')) {
                      ...
                      let lineHeight = strings.pixelHeight(fontName, fontStyleTextLines, fontSize)
                        ...
                        let textData = _drawMultilineText(itemText, adjustedBounds.x, adjustedBounds.y, adjustedBounds.width, adjustedBounds.height, fontName, fontSize, cfg.fontSizeCorrectionRatio, fontWeight, fontStyle, element.textAlignment, element.textPosition, cfg.doNotDrawTextIfBoxIsTooSmall, cfg.layoutGapTolerance)
            ...
            function _drawEdges (currentView, cfg) {
                ...
                connections.each (function (e) {
                    ...
                    if (e.prop("ASSOCIATION_DIRECTED") == "true" || (e.concept && e.concept.associationDirected))
                        drawLineArrows = true
                    ...
                    if (e.type == 'realization-relationship' || e.type == 'access-relationship' || e.type == 'flow-relationship') {
                    ...
                    if (e.prop("ASSOCIATION_DASHED") == "true" || e.prop("LINE_STYLE") == "DASHED" || (e.lineStyle == LINE_STYLE.DASHED))
                        lineStyle = ` stroke-dasharray="${cfg.dashSizeLine}" `
                    else if (e.lineStyle == LINE_STYLE.DOTTED)
                        lineStyle = ` stroke-dasharray="2" `
                    ...
                    if (relText.trim().length != 0) {
                        ...
                        let actualFontSize = (e.fontSize || _defaultFontSize) * cfg.fontSizeCorrectionRatio
                        ...
                        for (let relLineText of _sanitizeText(relText).split("\n")) {
                            ...
                            if (minEffectiveX == 0)
                                minEffectiveX = effectiveX
                            else
                                minEffectiveX = Math.min(minEffectiveX, effectiveX)
                        ...
                        let yoffset = 3.5
                        let effectiveY = relTextPoint.y + (currentLineHeight/yoffset)
                        let pos = 0

                        if (lineCount > 1)
                            effectiveY = relTextPoint.y - (currentLineHeight * lineCount / yoffset)

                        for (let relLineText of _sanitizeText(relText).split("\n")) {
                            textData = textData + `<tspan x="${minEffectiveX}" font-size="${actualFontSize}" y="${effectiveY}">${relLineText}</tspan>`
                            pos += (currentLineHeight)
                            effectiveY += currentLineHeight
                        }

                        let lineFillColor = e.prop("LINE_FILL_COLOR") || "#FFFFFF"
                        let textAnchor = "middle"

                        if (lineCount > 1) {
                            if (e.textAlignment == TEXT_ALIGNMENT.LEFT) {
                                textAnchor = "start"
                                minEffectiveX = minEffectiveX - (relTextWidth / 4)
                            } else if (e.textAlignment == TEXT_ALIGNMENT.RIGHT) {
                                textAnchor = "end"
                                minEffectiveX = minEffectiveX + (relTextWidth / 4)
                            }
                        }

                        let lineText = `<text x="${minEffectiveX}" y="${relTextPoint.y}" font-size="${actualFontSize}" font-weight="${fontWeight}" font-style="${fontStyle}"  stroke="${lineFillColor}" stroke-width="${strokeWidth + 4}" fill="${lineFillColor}" text-anchor="${textAnchor}">${textData}</text>`
                        lineText += `<text x="${minEffectiveX}" y="${relTextPoint.y}" font-size="${actualFontSize}" font-weight="${fontWeight}" font-style="${fontStyle}" fill="${fontColor}" text-anchor="${textAnchor}">${textData}</text>`
            ...
            function _labelTextLocation(textPosition, linePoints) {
                if (textPosition == CONNECTION_TEXT_POSITION.SOURCE) { // SOURCE
                    return linePoints[0]
                } else if (textPosition == CONNECTION_TEXT_POSITION.TARGET) { // TARGET
                    ...
                    if (linePoints.length % 2 == 0) {
                        ...
                        return {
                            x: p1.x + Math.floor(dw / 2),
                            y: p1.y + Math.floor(dh / 2)
                        }
                    }
