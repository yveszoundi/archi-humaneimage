...
var humaneImage = {
  "renderViewAsBase64": function (currentView, options) {
    ...
    function _drawMultilineText (text, x, y, widthLimit, heightLimit, fontSize, fontWeight, fontStyle, textAlignment, textPosition, padding) {
    ...
    function _drawElement (bounds, element, cfg) {
      ...
      if (itemText.length != 0) {
        if (!(element.type == 'junction')) {
          ...
          let textData = _drawMultilineText(itemText, bounds.x, bounds.y, bounds.width, bounds.height, fontSize, fontWeight, fontStyle, element.textAlignment, element.textPosition, cfg.padding)
