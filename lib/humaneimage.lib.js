...
load(__SCRIPTS_DIR__ + "lib/reflection.lib.js")

var humaneImage = {
  "renderViewAsBase64": function (currentView, options) {
    const _edraw2dJarPath = __SCRIPTS_DIR__ + "lib/edraw2d.jar"

    const _rgbColorByElementType = {
    ...
    function _calcImageSize(activeView, cfg) {
      ...
      $(activeView).find().each(function(e) {
        if (! (e.type.endsWith("-relationship") || e.type.endsWith("-connection")) )  {
      ...
      let result = { width : (maxX - minX), height: (maxY - minY) }
    ...
    function _drawMultilineText (text, x, y, widthLimit, heightLimit, fontSize, fontWeight, fontStyle, textAlignment, textPosition, padding, shouldNotDrawTextWheBoxIsTooSmall) {
      ...
      let dy = padding
      let lineHeight = strings.pixelHeight(fontSize) - (padding * 1.5)
      ...
      for (let newTextLine of newText) {
        ...
        if (textAlignment != 1 ) {  // LEFT
          let newTextLineWidth = strings.pixelWidth(newTextLine, fontSize)
    ...
    function _drawElement (bounds, element, cfg) {
      ...
      let adjustedBounds = {x : bounds.x , y: bounds.y, width: bounds.width, height: bounds.height}
      ...
      let fillColor = (element.fillColor) ? element.fillColor : (_rgbColorByElementType[element.type] || _defaultFillColor)
      ...
      let result = `<rect ` + additionalRectAttributes +  ` x = "${bounds.x}" y="${bounds.y }" width="${bounds.width}"  ${groupingStyle}  height="${bounds.height}" fill="${fillColor}" stroke-width="1" stroke="${lineColor}"/>`
      ...
      if (itemText.length != 0) {
        if (!(element.type == 'junction')) {
          ...
          let textData = _drawMultilineText(itemText, bounds.x, bounds.y, bounds.width, bounds.height, fontSize, fontWeight, fontStyle, element.textAlignment, element.textPosition, cfg.padding, cfg.doNotDrawTextWheBoxIsTooSmall)

          result += `<text style="font-weight:normal" x="${bounds.x + cfg.padding * 2}" y="${bounds.y + cfg.lineHeight}" font-size="${fontSize}" fill="${fontColor}" font-style="${fontStyle}" font-weight="${fontWeight}">${textData}</text>`
        }
      }
    ...
    function _lineArrow (fromx, fromy, tox, toy, color, strokeWidth, lineStyle) {
      ...
      function _lineArrowDelegate (fromx, fromy, tox, toy, color, strokeWidth, lineStyle) {
        ...
        result = result + (" L " + (tox - headlen * Math.cos(angle + Math.PI / 6)) + " " + (toy - headlen * Math.sin(angle + Math.PI / 6)))
        result = result + " Z\" stroke=\"" + color + "\"  fill-opacity=\"0\" stroke-linejoin=\"round\" stroke-width=\"" + strokeWidth + "\" " + lineStyle + "/>"
    ...
    function _rectanglePorts (r, cellSize, layoutGapTolerance) {
      let preferredInternal = cellSize * 5
    ...
    function _bestCoordinates (r1, r2, cfg) {
      ...
      let rp1 = _rectanglePorts(r1, cfg.cellSize, cfg.layoutGapTolerance)
      let rp2 = _rectanglePorts(r2, cfg.cellSize, cfg.layoutGapTolerance)
      rp1.push(rp1s)
      rp2.push(rp2s)
    ...
    function _drawEdges (currentView, cfg) {
      ...
      connections.each (function (e) {
        ...
        if (relText.trim().length != 0) {
          ...
          let relTextWidth = strings.pixelWidth(relText, actualFontSize)
          let currentLineHeight = strings.pixelHeight(actualFontSize)
          ...
          for (let relLineText of _sanitizeText(relText).split("\n")) {
            textData = textData + `<tspan x="${relTextPoint.x }" dy="1em">${relLineText}</tspan>`
    ...
    let cfg = {
      ...
      imageMargin                   : 50,
      cellSize                      : 3,
      lineHeight                    : strings.pixelHeight(_defaultFontSize),
    ...
    let svgText = `<svg width='${cfg.svgWidth}' version='1.1' height='${cfg.svgHeight}' style="background-color:white" xmlns='http://www.w3.org/2000/svg'>\n`
