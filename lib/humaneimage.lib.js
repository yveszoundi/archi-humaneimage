...
var humaneImage = {
  "renderViewAsBase64": function (currentView, options) {
    ...
    function _renderViewAsSvgBase64(currentView, cfg) {
      ...
      function _drawElement (bounds, element, cfg) {
        ...
        if (itemText.length != 0) {
          if (!(element.type == 'junction')) {
            ...
            let lineHeight = strings.pixelHeight(fontSize)
            ...
            result += `<text style="font-weight:normal" x="${bounds.x + cfg.padding * 2}" y="${bounds.y + lineHeight}" font-size="${fontSize}" fill="${fontColor}" font-style="${fontStyle}" font-weight="${fontWeight}">${textData}</text>`
          }
        }
      ...
      let svgBytes = new JString(svgText).getBytes()

      console.log(`\n+Exporting view(id:${currentView.id}, name:${currentView.name}) to SVG in base64 format\n`)

      return new JString(Base64.getEncoder().encode(svgBytes))
    }

    function _renderViewAsPngBase64 (svgTextInBase64, cfg) {
      let Base64 = Java.type("java.util.Base64")
      let JString = Java.type("java.lang.String")
      let svgTextBytes = Base64.getDecoder().decode(svgTextInBase64)
    ...
    let svgBase64 = _renderViewAsSvgBase64(currentView, cfg)

    if (options.format == "svg")
      return svgBase64

    return _renderViewAsPngBase64(svgBase64, cfg)
  }
