        ...
        for (let newTextLine of newText) {
          ...
          if (textAlignment != 1 ) {  // LEFT
            ...
            if (textAlignment == 2) { // MIDDLE
              horizontalTextPosition = (x + (widthLimit  / 2)) - (newTextLineWidth / 2) + gap
            } else {                  // RIGHT
      ...
      function _drawElement (bounds, element, cfg) {
        ...
        if (itemText.length != 0) {
          if (!(element.type == 'junction')) {
            ...
            let fontName = element.fontName || _defaultFontName
            let lineHeight = strings.pixelHeight(fontName, fontSize)
            let textData = _drawMultilineText(itemText, bounds.x, bounds.y, bounds.width, bounds.height, fontName, fontSize, fontWeight, fontStyle, element.textAlignment, element.textPosition, cfg.doNotDrawTextWheBoxIsTooSmall)

            result += `<text style="font-weight:normal" x="${bounds.x}" y="${bounds.y + lineHeight}" font-size="${fontSize}" font-family="${fontName}" fill="${fontColor}" font-style="${fontStyle}" font-weight="${fontWeight}">${textData}</text>`
          }
        }
