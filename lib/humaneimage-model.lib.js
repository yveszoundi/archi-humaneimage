...
var humaneImageModel = {
  "diagram": function (currentView, options) {
    ...
    function _extractDiagramModel(currentView, cfg) {
      ...
      function _drawMultilineText (text, x, y, widthLimit, heightLimit, fontName, fontSize, fontWeight, fontStyle, textAlignment, textPosition, doNotDrawTextIfBoxIsTooSmall, layoutGapTolerance) {
        ...
        for (let newTextLine of newText) {
          ...
          textSpans.push({ x: horizontalTextPosition + 1, y: dy, data: newTextLine })
      ...
      function _drawElement (bounds, element, level, cfg) {
        ...
        let lineWidth = (element.lineWidth || _defaultLineWidth)
        
        let figure = {
          id: element.id,
          roundedCorners: (cfg.RectRoundedEnabled = true),
          bounds: { x: adjustedBounds.x, y : adjustedBounds.y, width: adjustedBounds.width, height: adjustedBounds.height },
          lineColor: lineColor,
          lineWidth: lineWidth,
          fillColor: fillColor,
          borderStyle :  (element.type == "grouping") ? "dashed" : "straight",
        ...
        if (itemText.length != 0) {
          ...
          let fontStyleTextLines = _mapToJavaFontStyle(fontWeight, fontStyle)
          let lineHeight = strings.pixelHeight(fontName, fontStyleTextLines, fontSize)
          let horizontalTextAlignment = "MIDDLE"
          let verticalTextAlignment = "TOP"

          if (element.textAlignment != 2) {
            if (element.textAlignment == 1)
              horizontalTextAlignment = "LEFT"
            else
              horizontalTextAlignment = "RIGHT"
          }

          if (element.textPosition != 0) {
            if (element.textPosition == 1)
          ...
          let figureTextSpans = _drawMultilineText(itemText, adjustedBounds.x, adjustedBounds.y, adjustedBounds.width, adjustedBounds.height, fontName, fontSize, fontWeight, fontStyle, element.textAlignment, element.textPosition, cfg.doNotDrawTextIfBoxIsTooSmall, cfg.layoutGapTolerance)

          figureText.spans = figureTextSpans

          figure.text = figureText
        }
      ...
      function _drawVertices (currentView, cfg) {
        ...
        while (queue.length != 0) {
          ...
          if (!cfg.elementBoundsById.has(data.item.id)) {
            ...
            figures.push (_drawElement(itemBounds, data.item, data.level, cfg))
          }
      ...
      function _drawEdges (currentView, cfg) {
        ...
        connections.each (function (e) {
          ...
          let lineWidth = (e.lineWidth || _defaultLineWidth)
          ...
          if (relText.trim().length != 0) {
            ...
            for (let relLineText of _sanitizeText(relText).split("\n")) {
              ...
              figureConnectionText.spans.push({x: effectiveX, y: pos, data: relLineText})
              pos += (currentLineHeight)
            }

            figureConnection.text = figureConnectionText
          }

          figureConnections.push(figureConnection)
        })

        return figureConnections
      }
      ...
      let diagram = {
        ...
        defaultFontName: _defaultFontName,
        fillColor: cfg.imgBackgroundColor,
        bounds: {
          x: 0, y: 0, width: cfg.svgWidth, height: cfg.svgHeight
        },
        figures: _drawVertices(currentView, cfg),
        connections: _drawEdges(currentView, cfg)
      }
    ...
    return _extractDiagramModel(currentView, cfg)
  }
